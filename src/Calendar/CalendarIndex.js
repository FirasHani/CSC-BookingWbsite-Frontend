    ...
    <>
      <section className="relative z-10 bg-white py-[120px] ">
        <div className="mx-auto px-4 lg:container">
          <div className="mb-[30px] flex items-center justify-between rounded-lg border border-stroke bg-gray-2 py-3 pl-[30px] pr-4 ">
            <p className="text-base font-semibold text-dark sm:text-xl">
              December 2026
            </p>
            <div className="relative z-20 inline-flex rounded-[5px] bg-white ">
              <select
                name=""
                id=""
                className="relative z-20 h-11 appearance-none rounded-[5px] border border-stroke bg-transparent pl-5 pr-10 text-dark outline-none "
              >
                <option value="">Week</option>
                <option value="">Month</option>
                <option value="">Year</option>
              </select>
              <span className="absolute right-5 top-1/2 z-10 -translate-y-1/2 text-dark ">
                <svg
                  width="11"
                  height="6"
                  viewBox="0 0 11 6"
          ...
          <div className="w-full max-w-[1000px] overflow-x-auto bg-white ">
...
const Day = ({ active, number, eventTitle, eventDate }) => {
    ...
    return (
      <td 
        className="ease relative h-28 w-10 cursor-pointer border border-stroke p-1 transition duration-500 hover:bg-gray-2 md:h-[125px] lg:w-28 2xl:w-40"
        onClick={handleClick}
      >
        <div className="mx-auto flex h-24 w-10 flex-col overflow-hidden sm:w-full md:h-40 md:w-20 lg:w-28 2xl:w-40">
          ...
          {active && (
            <div className="bottom md:h-30 group h-16 w-full flex-grow cursor-pointer py-1">
              <span className="text-dark group-hover:text-primary md:hidden">
                More
              </span>
              <div className={`event ${isPopupVisible ? 'visible opacity-100' : 'invisible opacity-0'} absolute left-2 z-10 mb-1 w-[200%] rounded border-l-[3px] border-primary bg-gray-2 px-[18px] py-[6px] text-left transition-opacity duration-300 md:visible md:w-[190%] md:opacity-100`}>
                <span className="event-name block text-sm font-medium text-dark ">
                  {eventTitle}
                </span>
                <span className="time text-sm text-body-color ">
                  {eventDate}
                </span>
              </div>
            </div>
        ...
        {isPopupVisible && (
          <div className="popup-overlay fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-20" onClick={() => setPopupVisible(false)}>
            <div className="popup-content bg-white p-4 rounded shadow-lg">
              <h2 className="text-xl font-semibold">{eventTitle}</h2>
              <p>{eventDate}</p>
              <button className="mt-2 px-4 py-2 bg-blue-500 text-white rounded" onClick={() => setPopupVisible(false)}>Close</button>
            </div>
          </div>
        )}
      </td>
